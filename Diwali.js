...
let fireworks = [], particles = [], hue = 120, timerTick = 0, timerTotal = 80;
...
function Particle(x,y){
  ...
  while(this.coordinateCount--) this.coordinates.push([this.x,this.y]);
  this.angle=random(0,Math.PI*2); this.speed=random(1,10); this.friction=0.95; this.gravity=1;
  this.hue=random(hue-50,hue+50); this.brightness=random(50,80); this.alpha=1; this.decay=random(0.015,0.03);
}
...
function loop(){
  ...
  if(timerTick>=timerTotal){fireworks.push(new Firework(canvas.width/2,canvas.height,random(0,canvas.width),random(0,canvas.height/2))); timerTick=0;} else timerTick++;
}
setInterval(()=>{fireworks.push(new Firework(canvas.width/2,canvas.height,random(0,canvas.width),random(0,canvas.height/2)));},1500);
loop();
